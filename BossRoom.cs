...
public class BossRoom : MonoBehaviour
{
   //掉落宝箱功能
    ...
    public GameObject Gobulin_Witch;
    ...
    void Update()
    {
            ...
            if(count_time == 3)
            {
                ...
                if(is_Finished == true && alreadyDestroy == false)//在第三波之后
                {
                    ...
                    alreadyDestroy = true;
                     ...
                     //创造boss
                        Boss = Instantiate(Gobulin_Witch,new Vector3(center_x,center_y,0),Quaternion.identity);
                        Boss.GetComponent<Gobulin_Witch>().width = Width;
                        Boss.GetComponent<Gobulin_Witch>().height = Height;
                        Boss.GetComponent<Gobulin_Witch>().theCenterPos = new Vector3(center_x,center_y,0);
        ...
        if(Boss != null)
        {
                if(Boss.GetComponent<Gobulin_Witch>().health <= 0 && CreatedtheDoor == false)
                {
                    Destroy(Door1_real);
                    MovePos.localPosition = new Vector3(MovePos.localPosition.x - 0.5f,MovePos.localPosition.y,0);
                    Instantiate(TreasureBox,MovePos.localPosition,Quaternion.identity);
                    Instantiate(PassDoor,new Vector3(center_x,center_y,0),Quaternion.identity);
                    CreatedtheDoor = true;
                }
        }
